...
let links = [
  "https://upload.wikimedia.org/wikipedia/commons/1/13/Michelangelo%2C_Creation_of_Adam_06.jpg",
...
async function analyzeImage(img) {
  ...
  x.font =
    scale.value *
      (display.value == "chart" ? 0.75 : display.value == "mosaic" ? 1.5 : 1) +
    "px Segoe UI Emoji";
  ...
  for (let i = 0; i < ROWS * COLS; i += 1) {
    ...
    let col = i % COLS;
    let buffer = hiddenX.getImageData(
      col * pSize.value,
      row * pSize.value,
      pSize.value,
      pSize.value
    ...
    let isBlack =
      colorInfo.rgb[0] == 0 && colorInfo.rgb[1] == 0 && colorInfo.rgb[2] == 0;
    ...
    bestEmoji =
      hueMap[lightness].emojis[
        Math.floor(hueMap[lightness].emojis.length * Math.random())
      ];
  ...
  var pix = pixels.sort((a, b) => (Math.random() > 0.5 ? 1 : -1));
  for (emo of pix) {
...
function buildEmojiMap() {
  ...
  emojis.forEach((emo, index) => {
    ...
    emo.isSupported =
      !isGray &&
      colors.transparentPixels /
        (colors.totalPixels + colors.transparentPixels) <
        Number(emptySpace.value);
  });
...
function getColorInfo(arr, ignoreFill) {
  ...
  let rgb =
    totalPixels == 0
      ...
      : sums
          .filter((el, i) => i < 3)
          .map((s) => Math.round(Math.sqrt(s / totalPixels)));
